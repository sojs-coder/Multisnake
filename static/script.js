...
var how_to_play = {
  'normal': 'Classic mode plays much like classic snake, but with the addition of walls and boosts. Move around using WASD or arrow keys. Use SPACE to boost, and get to 10 apples to win.',
...
window.log = (html, cb) => {
  ...
  document.getElementById('alert-box').innerHTML += `<button id = 'close' class = '__spectate'>Close</button><button id = '__spectate' class = '__spectate'>Spectate</button>`;
  if (document.getElementById('__spectate')) {
    document.getElementById('__spectate').onclick = (e) => {
      alert('CTRL+R or F5 to go back to playing')
  ...
  document.getElementById('close').onclick = (e) => {
    document.getElementById('alert-box').style.display = "none";
    if (cb !== document.location.reload) {
      cb();
    } else {
...
socket.on('death', (id) => {
  ...
  if (id == window.thisSnakeID) {
    ...
    log(`<h1>You Died!</h1><p>Your final score was ${window.game.score}. Your time was ${time} seconds. Nice Job</p><h3>Share your score...</h3>     <h3>Remember to Join our community...</h3><div style = "font-size:50px"><a style = "padding: 25px" href = "//discord.gg/Np7vBvEtp2"><i class="fab fa-discord"></i></a><a style = "padding: 25px" href = "https://github.com/sojs-coder/Multisnake"><i class="fab fa-github"></i></a><br></div>`, document.location.reload);
  }
});
...
socket.on('win', (snake) => {
  ...
  if (snake.id == window.thisSnakeID) {
    var time = getFormatGameTime();
    log(`<h1>You Win!</h1><p>You won in  ${time} seconds. <b>Nice Job</b></p><h3>Share your win with our community</h3><a style = "padding: 25px" href = "//discord.gg/Np7vBvEtp22"><i class="fab fa-discord"></i></a><a style = "padding: 25px" href = "https://github.com/sojs-coder/Multisnake"><i class="fab fa-github"></i></a><br></div>`, document.location.reload);
  } else {
    // the snake is not the user. tell him who won and what place he got.
    var time = getFormatGameTime();
    log(`<h1>You Finished ${(parseInt(currplace) + 1).toString()}</h1>The winner of this round was "${snake.username}"<p><p>Your final score was ${window.game.score}. Your time was ${time} seconds. Nice Job...</p><h3>Share your score with our community</h3><div style = "font-size:50px"><a style = "padding: 25px" href = "//discord.gg/Np7vBvEtp2"><i class="fab fa-discord"></i></a><a style = "padding: 25px" href = "https://github.com/sojs-coder/Multisnake"><i class="fab fa-github"></i></a></div>`, document.location.reload);
  }
});
if (type == 'redgreen') {
  socket.on('color', (d) => {
  ...
  allSnakes.forEach((snake) => {
    ...
    if (snake !== "dead_snake") {
      if (snake.it) {
        ...
        } else if(snake.type == 'BOT'){
          propagateSnake(snake.blocks, "#800080", false);
        }else{
